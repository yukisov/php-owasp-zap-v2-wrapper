...
class Users {
  ...
  {
    $arr_users = $this->usersList($context_id);
    if (in_array($name, $arr_users)) {
      return;
    }
  ...
  {
    $str_enabled = ($enabled) ? 'true' : 'false';

    try {
      $resJsonObj = $this->zap->users->setUserEnabled($context_id, $user_id, $str_enabled, $api_key);
      $this->zap->expectOk($resJsonObj);
    } catch (\Zap\ZapError $e) {
